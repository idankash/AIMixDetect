"""
This script is designed to create the null distribution and run the pipeline in one go.
...
def main():
    ...
    INPUT_FILE_TEST = params['input-file-test']
    ...
    # Create the split
    df_null = pd.read_csv(INPUT_FILE)
    ...
    for get_edits in [False, True]:
        ...
        for i in range(len(chunks['text'])):
            ...
            output_file = f"{OUTPUT_PATH}/edit_ratio_{edit_ratio}/{results_folder}/{name}.csv"
...
#                 print(df[df['mask']])
            ...
            results_filename = f"{OUTPUT_PATH}/edit_ratio_{edit_ratio}/{results_folder}/article_{i}_{suffix_file_name}_results.csv"
            print(results)
